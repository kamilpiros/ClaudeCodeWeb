import { z } from "zod";
import type { ActionItemRow, CompanyRow, NoteRow } from "./types";
...
export async function confirmCapture(
  ...
  if (existing) {
    ...
    if (statusChanged || draft.pass_reason) {
      stmts.push(
        db
          .prepare(
            `UPDATE companies
             SET status = ?, pass_reason = COALESCE(?, pass_reason), updated_at = ?
             WHERE id = ?`,
          )
          .bind(
  ...
  return { company: company ?? null, note, action_items: items.results };
}
